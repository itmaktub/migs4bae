...
?>
	<div id="content">
		...
		<div id="x-photo">
			...
			<div id="photo-item">
				<img id="zoomimg" class="zoomable" src="<?php echo photo_url($pthis->storage, $pthis->name);?>" width="450" alt="<?php echo $pthis->title; ?>" />
			</div><!-- #photo-item -->
			...
			<div id="photo-meta">
				<ul>
					...
					<li class="outlink"><span class="pmeta">外链地址：<?php //var_dump($pthis);?></span>
						<a href="<?php echo "http://$pthis->storage.bcs.duapp.com/thumb/$pthis->name";//thumb_url($pthis->storage, $pthis->name);?>">200</a>
						<a href="<?php echo photo_url($pthis->storage, $pthis->name);?>">original</a>
					</li>
					<li><span class="pmeta">Wordpress：</span><textarea rows="1">&lt;img src="<?php echo photo_url($pthis->storage, $pthis->name);?>" alt="<?php echo $pthis->title; ?>" class="aligncenter size-full" /&gt;</textarea></li>
                    <li><span class="pmeta">所属相册：</span>
					<?php
					$category_info = $migs_db->get_results("SELECT * FROM mi_category WHERE id='$pthis->category' LIMIT 0,1");
					$current = $category_info[0];
					if ( $LOGIN ) { ?>
						<span id="category-<?php echo $pthis->uid;?>" class="editable" title="<?php echo $current->desc; ?>"><?php echo $current->cate_name; ?></span>
						<!--<input id="editcategory-<?php echo $pthis->uid;?>" class="edititle" type="text" value="<?php echo $pthis->category; ?>" onblur="update_category('<?php echo $pthis->uid;?>');" />-->
						<?php $category_info = $migs_db->get_results("SELECT id,cate_name FROM mi_category ORDER BY id DESC");?>
                        <select id="editcategory-<?php echo $pthis->uid;?>" name="category" class="edititle" onblur="update_category('<?php echo $pthis->uid;?>');">
                            <?php
							foreach($category_info as $single){?>
							<option value="<?php echo $single->id; ?>"<?php if($single->id == $pthis->category){?> selected="selected"<?php }?>><?php echo $single->cate_name; ?></option>	
							<?php }?>
                         </select>
					<?php } else { ?>
						<span  title="<?php echo $current->desc; ?>"><?php echo $current->cate_name; ?></span>
					<?php } ?>
					</li>
					<?php if ( $LOGIN ) { ?>
