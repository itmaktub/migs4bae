		...
		<div id="x-photo">
			...
			<form id="f-stor" class="form" action="<?php echo SITE_URL.'do.php?action=stor'?>" method="post">
				...
				<?php
					...
					$my_storages=json_decode($storage->list_bucket()->body);
					for ( $i = 0; $i < 1; $i++ ) {
				?>
				<p>
                    ...
                    <?php
						if( !isset($stor[$i])) continue;
						foreach($my_storages as $key => $single){
							if($single->bucket_name==$stor[$i])
								break;
						}
					?>
					<span class="stor-usage">总容量<?php echo number_format($single->total_capacity/(1024*1024),2);?>MB，已经使用<?php echo number_format($single->used_capacity/(1024*1024),2);?>MB，还有<?php echo number_format(($single->total_capacity-$single->used_capacity)/(1024*1024),2);?>MB可以使用，使用了<?php echo number_format($single->used_capacity*100/$single->total_capacity,2);?>%的存储空间。</span>
				</p>
				<?php } ?>
				<p>
